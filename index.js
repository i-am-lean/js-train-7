...
let rangeObject = {  
  ...
  next() {// Створюємо метод "next" який визначає поведінку при кожній ітерації
    if(this.current < this.to) {// Використовуйте if
      return {// Якщо current менше to, повертаємо об'єкт з властивістю "value",що містить поточне значення ,
...
let myObject = {
  ...
  [Symbol.iterator]: function(){// Оголошення методу Symbol.iterator для об'єкта "myObject"
    return this;//Повертаємо this
  },
